...
import { addToOutbox, getOutbox } from '../utils/idbHelper';
 ...
 */
const generateIdempotencyKey = async (method, url, data) => {
...
api.interceptors.request.use(
  async (config) => {
    ...
    if (!navigator.onLine && isWriteMethod && !config.skipOfflineInterceptor) {
      ...
      // Throw mock error to route to response interceptor error handler
      throw { isOfflineMock: true, data: responseData };
    }
...
api.interceptors.response.use(
  (response) => response,
  (error) => {
    // If it's a mocked offline response, resolve it as a successful response
    ...
    if (error.response && error.response.status === 401) {
      // Handle unauthorized (e.g., remove session indicator)
