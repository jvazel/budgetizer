...
- **Personnalisation & Ordonnancement du Dashboard (`DashboardCustomizerSheet`)** : Un bouton dédié dans l'en-tête ouvre un tiroir tactile permettant de réordonner dynamiquement les 8 widgets par glissement/flèches (monter/descendre) et de masquer/afficher n'importe quel bloc selon les besoins de l'utilisateur, avec sauvegarde automatique dans les préférences locales.
- **Raccourcis Tactiles Adaptatifs (`ShortcutsWidget`)** : Grille dynamique d'accès directs vers 8 modules clés (Budgets, Épargne, Analyses, Abonnements, Scores, Assistant IA, Échéances, Virements), lisible et contrastée en mode clair comme en mode sombre.
- **Carte des Statistiques Temporelles (Timeframe Statistics Card)** : Un composant compact à onglets qui regroupe la synthèse mensuelle (Revenus, Dépenses, Solde net) et le graphique de vélocité hebdomadaire (Sparkline courbe d'aire Recharts des 7 derniers jours) dans un espace visuel unifié et interactif. L'onglet actif s'anime avec une pilule orange vif (`bg-amber-600` / `#d97706`).
