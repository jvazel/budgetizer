...
- **Tiroir de Navigation (`MenuSheet.jsx`)** : Un menu coulissant moderne est déployé de manière fluide grâce à `framer-motion` (physique de ressort). Il dispose de halos de lumière diffuse (`blur-[60px] bg-copper/5` et `bg-purple/5`). Son architecture a été sécurisée avec des z-indexes rigoureux (`z-40` pour le backdrop, `z-50` pour le tiroir) pour éviter que le flou d'arrière-plan ne se superpose au menu en thème clair, et son arrière-plan utilise la classe solide `bg-surface` (blanc opaque en thème clair). Les boutons et icônes inactives exploitent `bg-surface-2` et `hover:bg-surface-2/50` pour assurer une visibilité et un retour tactile irréprochables dans tous les modes. Les liens de menu sont structurés et ordonnés en 4 groupes selon leur fréquence d'usage (*Mon Quotidien*, *Mon Budget & Projets*, *Analyses & IA*, *Outils & Options*), et disposent de micro-animations de translation latérale au survol et d'un indicateur d'état actif cuivré.
- **Liste Optimisée des Transactions (`TransactionList.jsx`)** : Le composant a été restructuré pour s'adapter dynamiquement aux contraintes mobiles et gérer le contexte de filtrage :
  ...
  - **Montants colorisés** : Les montants utilisent `text-danger` (rouge) pour les dépenses et `text-accent` (vert) pour les revenus, remplaçant l'ancien `text-primary` pour les dépenses.
  - **Swipe-to-Edit & Swipe-to-Delete** : Le glissement gauche (Swipe Left) exposait 1 seul bouton (supprimer, 80 px). Il expose désormais **2 boutons côte à côte** : ✏️ Modifier (accent, 80 px) et 🗑️ Supprimer (danger, 80 px). Le `dragConstraints.left` est passé de `-80` à `-160` pixels. L'import `Pencil` et `CheckCircle2` (Lucide React) a été ajouté.
  ...
  - **Gestion dynamique des signes pour les virements** : Affiche les transferts en débit négatif (`-` et rouge `text-danger`) par défaut ou lorsqu'on filtre sur le compte courant source, mais les affiche en crédit positif (`+` et vert `text-accent`) lorsqu'on filtre sur le compte destinataire.
- **Formulaire de Saisie Rapide (`TransactionFormSheet.jsx`)** — Améliorations UX Speed :
  ...
  - **Solde résultant à la volée** : Un `useMemo` recalcule `projectedBalance = account.balance + delta` à chaque changement de `amount`, `accountId` ou `type`. Rendu sous `<AmountInput>` via un badge `rounded-full` coloré.
  - **Indicateur de Budget Inline** : `useBudgets()` est importé dans le composant. Un `useMemo` calcule `activeBudget` en matchant `b.categoryId._id === categoryId`. Le rendu conditionnel s'affiche entre la grille Compte/Catégorie et le sélecteur de date si `activeBudget && type === 'expense'`. La barre SVG utilise `Math.min(pct, 100)` pour ne pas déborder visuellement. L'alerte ⚠️ s'affiche si `projectedSpent > limit`.
  - **Navigation directe Compte ↔ Catégorie** : Les panels `activePanel === 'account'` et `activePanel === 'category'` ont tous deux reçu une barre d'onglets `[Compte] [Catégorie]` avec l'onglet actif en `bg-accent text-white` et l'inactif cliquable pour switcher.
  - **Date accordéon** : `showDatePicker` state + bouton badge `"📅 Aujourd'hui"` / date formatée. Le champ `<input type="date">` est conditionnel.
...
- **Titres de Page Collapsibles et En-tête Collante (`AppShell.jsx` / `Home.jsx`)** : Intégration d'un écouteur de défilement (`window.scrollY`) pour basculer dynamiquement l'opacité et l'affichage des titres de page dans le header collant. Pour maintenir le header collant (`sticky`) lors du défilement sans blocage par l'overflow sur mobile, le conteneur racine d'AppShell utilise la classe CSS `overflow-x-clip` (qui ne définit pas un nouveau conteneur de défilement pour le positionnement collant, contrairement à `overflow-x-hidden`).
