...
- **Liste Optimisée des Transactions (`TransactionList.jsx`)** : Le composant a été restructuré pour s'adapter dynamiquement aux contraintes mobiles et gérer le contexte de filtrage :
  ...
  - **Typographie des Montants et Alertes** : Les montants sont formatés avec la police `.font-premium-numbers` en taille `text-sm sm:text-base` et graisse `font-extrabold`. Les revenus sont en vert émeraude (`text-accent`), les dépenses importantes qui dépassent le seuil d'alerte de l'utilisateur s'affichent en rouge vif (`text-danger`), et les dépenses standard sont en blanc/bleuté sobre (`text-primary/80`).
  - **Swipe-to-Edit & Swipe-to-Delete** : Le glissement gauche (Swipe Left) expose désormais deux boutons côte à côte : ✏️ Modifier (bleu accent, 80 px) et 🗑️ Supprimer (rouge danger, 80 px). Le `dragConstraints.left` est passé à `-160` pixels pour accueillir les deux boutons.
...
- **Tableau de Bord (`Home.jsx`) — Restructuration Structurelle** :
  ...
  - **Carrousel des Comptes Glassmorphic** : Les cartes de compte sont passées sur un style glassmorphic brillant (`.bg-surface-glass` et `.backdrop-blur-md`) avec des reflets et des bordures semi-transparentes (`border-white/10`).
  - **Voyant de Synchronisation Live** : Ajout d'une balise verte pulsante (`.animate-pulse-live`) et labellisée `"Live"` sur chaque carte de compte pour confirmer l'état de synchronisation en temps réel avec la base de données.
...
- **Confirmation unifiée (`ConfirmModal.jsx` / `Transactions.jsx` / `SettingsPage.jsx`)** : Le composant réutilisable de dialogue de confirmation `<ConfirmModal>` gère désormais les avertissements système critiques de suppression (suppression de transactions, de filtres personnalisés dans Transactions, effacement des données et suppression de compte dans SettingsPage) avec trois configurations thématiques de danger (danger, warning, info) et effets d'entrée élastiques.
