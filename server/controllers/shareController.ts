...
// @access  Private
export const getShares = async (req: AppRequest, res: AppResponse) => {
  try {
...
// @access  Private
export const createShare = async (req: AppRequest, res: AppResponse) => {
  try {
...
// @access  Private
export const updateShare = async (req: AppRequest, res: AppResponse) => {
  try {
...
// @access  Private
export const deleteShare = async (req: AppRequest, res: AppResponse) => {
  try {
    const share = await Share.findById(req.params.id);
    if (!share) {
      return res.status(404).json({ message: 'Partage non trouvé.' });
