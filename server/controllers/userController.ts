...
export const updateProfile = async (req: AppRequest, res: AppResponse) => {
...
// 2. Change Password
export const updatePassword = async (req: AppRequest, res: AppResponse) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
export const updatePreferences = async (req: AppRequest, res: AppResponse) => {
  try {
    const { 
      currency, 
      theme, 
...
export const deleteMyAccount = async (req: AppRequest, res: AppResponse) => {
...
export const clearMyData = async (req: AppRequest, res: AppResponse) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
    const userId = req.user!.id;
