...
// @access  Private
export const getMonthlyReport = async (req: AppRequest, res: AppResponse) => {
  try {
    const userId = req.user!.id;
    const monthKey = String(req.params.monthKey); // Format: "YYYY-MM" (ex: "2026-05")
    ...
    const categoryTxSums: Record<string, number> = {}; // catId -> totalAmount
    const categoryTxCounts: Record<string, number> = {}; // catId -> count
    ...
    const categoryTxAverages: Record<string, number> = {};
    Object.keys(categoryTxSums).forEach(catId => {
      const sum = categoryTxSums[catId] || 0;
      const count = categoryTxCounts[catId] || 1;
    ...
    const catExpensesM: Record<string, number> = {};
    txsM.forEach(tx => {
    ...
    const catHistoryTotal: Record<string, number> = {};
    const catHistoryMonths: Record<string, Set<string>> = {}; // catId -> Set de mois uniques
    historyTxs.forEach(tx => {
      if (tx.type === 'expense' && tx.categoryId) {
        const catId = tx.categoryId.toString();
    ...
    const catHistoryAverages: Record<string, number> = {};
    Object.keys(catHistoryTotal).forEach(catId => {
      const total = catHistoryTotal[catId] || 0;
      const numMonths = catHistoryMonths[catId]?.size || 1;
      catHistoryAverages[catId] = total / numMonths;
    });

    // Budgets définis (budgets déjà chargé)
    let exceededBudgetName: string | undefined = undefined;
    let exceededBudgetPercent = 0;
    let wellManagedBudgetName: string | undefined = undefined;
    ...
    budgets.forEach(b => {
      if (b.period === 'monthly' && b.categoryId) {
        ...
        if (pct > 100 && pct > exceededBudgetPercent) {
          ...
          exceededBudgetName = (b.categoryId as { name?: string }).name;
        } else if (pct < 85 && (pct < wellManagedBudgetPercent || wellManagedBudgetPercent === 0) && spent > 0) {
          wellManagedBudgetPercent = pct;
          wellManagedBudgetName = (b.categoryId as { name?: string }).name;
        }
      }
    });

    // Variations par catégorie
    let decreasedCategoryName: string | undefined = undefined;
    let decreasedCategoryPct = 0;
    let increasedCategoryName: string | undefined = undefined;
    ...
    Object.keys(catExpensesM).forEach(catId => {
      ...
      if (avg && avg > 10) { // Seuil pour ignorer les petites catégories
        ...
        if (pctDiff < -15 && pctDiff < decreasedCategoryPct) {
          ...
          if (tx) decreasedCategoryName = (tx.categoryId as { name?: string }).name;
        } else if (pctDiff > 30 && pctDiff > increasedCategoryPct) {
          increasedCategoryPct = pctDiff;
          const tx = txsM.find(t => t.categoryId && t.categoryId._id.toString() === catId);
          if (tx) increasedCategoryName = (tx.categoryId as { name?: string }).name;
        }
      }
    });
    ...
    if (outlierTx) {
      ...
      const txDesc = displayName && displayName.trim() !== ''
        ? `pour "*${displayName}*"`
        : 'sans description';
      const catObj = outlierTx.categoryId as { name?: string } | null;
    ...
    logger.error('Erreur génération rapport mensuel:', { error: (error as Error).message });
