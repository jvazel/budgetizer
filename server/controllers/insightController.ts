...
const toObjectId = (id: string | import('mongoose').Types.ObjectId) => {
 ...
 * @access  Private
 */
export const getInsights = async (req: AppRequest, res: AppResponse) => {
  try {
    ...
    if (req.query.threshold) {
      const val = parseFloat(String(req.query.threshold));
      if (!isNaN(val)) {
    ...
    logger.error('Error generating insights:', { error: (error as Error).message });
